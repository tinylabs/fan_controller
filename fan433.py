...
#   - Preamble "00000"
#   - Between repeats: 13 ones + 2 zeros + 5 zeros (preamble) => 7 zeros before next '1'
#   - Stream ends with exactly 40 zeros (configurable)
...
# TODO: Speed 0 is likely broken for addr!=111(7)
D0 = {**{s:0x407d for s in range(0,10)}, 0:0x40d1, 10:0x4040}  # a0 (LSB)
...
DEFAULT_REPEATS    = 8
DEFAULT_IDLE1      = 13        # 13 ones between repeats (per your note)
DEFAULT_IDLE0      = 2         # 2 zeros; with preamble's 5 zeros => 7-zero gap
DEFAULT_GAP_ZEROS  = 7         # target zeros before next '1'
DEFAULT_END_ZEROS  = 40        # ensure the final transmission ends with 40 zeros
...
def set_trailing_zeros(s: str, n: int) -> str:
    """Return string that ends with exactly n zeros (trim or pad as needed)."""
    core = s.rstrip('0')
    return core + ('0' * n)
...
def build_tx_bits(speed: int, addr: int,
                  ...
                  end_zeros: int = DEFAULT_END_ZEROS,
                  auto_gap: bool = True) -> str:
    """
    Build (preamble + body + '1'*idle1 + '0'*idle0) * repeats, then ensure the
    entire transmission ends with exactly `end_zeros` zeros.
    If auto_gap=True, choose idle0 so that the number of zeros before the next '1'
    equals `gap_zeros`. With preamble="00000" and gap_zeros=7 -> idle0=2.
    """
    if auto_gap:
    ...
    stream = burst * repeats

    # Force exactly `end_zeros` trailing zeros
    stream = set_trailing_zeros(stream, end_zeros)
    return stream
...
# If hex convert else return binary string
def convert_hex (data: str):

    # Strip new lines
    data = data.replace('\n', '')
    
    # If already binary return
    if set (list (data)) == set (['1', '0']):
        return data
    # Convert to hex
    binary = []
    for n in data:
        binary += list(format (int (n, 16), '04b'))
    return ''.join(binary)        
...
def main():
    ...
    # Decode for URH
    p_dec_urh = sub.add_parser('decode-urh', help='Decode to binary 3bit addr, 4bit speed')
    p_dec_urh.add_argument('-b','--bs', required=True, help='bitstream')

    # Encode for URH
    p_enc_urh = sub.add_parser('encode-urh', help='Encode to bitstream')
    p_enc_urh.add_argument('-b','--bs', required=True, help='bitstream')
    p_enc_urh.add_argument('--repeats', type=int, default=DEFAULT_REPEATS)
    p_enc_urh.add_argument('--preamble', default=DEFAULT_PREAMBLE)
    p_enc_urh.add_argument('--idle-ones', type=int, default=DEFAULT_IDLE1)
    p_enc_urh.add_argument('--idle-zeros', type=int, default=DEFAULT_IDLE0)
    p_enc_urh.add_argument('--gap-zeros', type=int, default=DEFAULT_GAP_ZEROS, help='target zeros before next 1 (auto mode)')
    p_enc_urh.add_argument('--end-zeros', type=int, default=DEFAULT_END_ZEROS, help='force transmission to end with exactly this many zeros')
    p_enc_urh.add_argument('--no-auto-gap', action='store_true', help='disable automatic gap calculation')
    ...
    p_wave.add_argument('--end-zeros', type=int, default=DEFAULT_END_ZEROS, help='force transmission to end with exactly this many zeros')
