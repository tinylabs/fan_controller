...
# - Treats each repeat as a 16-bit codeword (decoded from (1,0) pairs; tail-aligned)
# - Encodes/decodes with cw = W @ features(speed,addr) mod 2
...
# Framing (matches captures)
DEFAULT_PREAMBLE = "000001010101010"
DEFAULT_REPEATS  = 8
DEFAULT_IDLE1    = 12  # '1's between repeats
DEFAULT_IDLE0    = 7   # '0's after those 1s
...
def build_W(BASE, D0, D1, D2):
    """Build 16x48 weight matrix W from BASE/D* per speed."""
    # Feature indices
    BIAS = 0
    A0, A1, A2 = 1, 2, 3
    S   = lambda s: 4 + s        # 4..14
...
def encode_affine(speed: int, addr: int) -> int:
    ...
    f = [0]*48
    f[0] = 1  # bias (unused by our construction but kept for generality)
...
# ---------- Wire mapping (symbol→waveform and back) ----------
def symbols_to_wave(bits16: str) -> str:
    """Map symbol '0' -> '10', '1' -> '100'."""
...
def build_tx_bits(speed: int, addr: int,
                  ...
                  idle0=DEFAULT_IDLE0, repeats=DEFAULT_REPEATS) -> str:
