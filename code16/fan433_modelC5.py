...
# - Transmit-side symbol order: MSB-first (matches your recordings)
...
DEFAULT_PREAMBLE = "00000"          # five zeros; the following 1010... is the start of the body
DEFAULT_REPEATS  = 8
DEFAULT_IDLE1    = 12               # 12 ones between repeats
DEFAULT_IDLE0    = 3                # 3 zeros; with next preamble's 5 zeros => 8-zero gap
...
def tail_decode_16(seg_bits: str):
    ...
    This skips preamble and lands on the 16 codeword symbols.
    """
    runs = rle(seg_bits)
    syms = []
...
def build_tx_bits(speed: int, addr: int,
    ...
    With preamble="00000", idle1=12, idle0 defaults to 3 (12 ones + 3 zeros + 5 zeros = 8 zeros gap).
    """
    if gap8:
        idle0 = max(0, 8 - leading_zeros(preamble))  # for "00000" -> 8 - 5 = 3
