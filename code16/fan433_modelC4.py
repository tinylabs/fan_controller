...
# - Decodes raw 0/1 captures into unique 16-bit codewords and (speed, addr)
# - Encodes (speed, addr) into on-air waveform with correct preamble/spacing
# - Transmit-side symbol order: **LSB-first** to match recordings
from pathlib import Path
import argparse
from itertools import groupby

# ---------- Protocol constants learned from your captures ----------
# BASE codewords (address 000), per speed 0..10
BASE = {
...
DEFAULT_PREAMBLE = "000001010101010"  # 5 zeros then alternating '01'
DEFAULT_REPEATS  = 8
DEFAULT_IDLE1    = 12                 # 12 ones between repeats
DEFAULT_IDLE0    = 3                  # with 5 leading zeros in preamble -> 3 + 5 = 8 zero gap
...
def tail_decode_16(seg_bits: str):
    ...
    This skips preamble symbols and lands on the 16 codeword symbols.
    """
    runs = rle(seg_bits)
    syms = []
...
def encode_affine(speed: int, addr: int) -> int:
    ...
    # Build feature vector f (length 48)
    f = [0]*48
    f[0] = 1
...
# ---------- Wire mapping ----------
def symbols_to_wave(bits16: str, lsb_first: bool = True) -> str:
    """
    Map 16 symbol bits to on-air waveform using **LSB-first** by default.
    Symbol mapping: 0 -> '10', 1 -> '100'
    """
    b = bits16[::-1] if lsb_first else bits16  # LSB-first on air
    return ''.join('100' if s == '1' else '10' for s in b)
...
def build_tx_bits(speed: int, addr: int,
                  ...
                  gap8: bool = True,
                  lsb_first: bool = True) -> str:
    """
    Build (preamble + body + '1'*idle1 + '0'*idle0) * repeats.
    If gap8=True, set idle0 so there are exactly 8 zeros before the next preamble's first '1'.
    """
    if gap8:
        idle0 = max(0, 8 - leading_zeros(preamble))  # e.g., "000001..." -> 8-5 = 3
    cw = encode_affine(speed, addr)
    bits16 = int_to_bits(cw, 16)
    body = symbols_to_wave(bits16, lsb_first=lsb_first)
...
def main():
    ...
    p_wave.add_argument('--msb-first', action='store_true', help="send symbols MSB-first instead of LSB-first")
